const WSUtilLoader = require("./ws-util");
let WSUtil;
let TOKEN;
let sourceTokens;
...
function parseParam(tokenizer) {
  ...
  while (tokenizer.hasNext()) {
    ...
    paramStr += token;
    if (token == TOKEN.N) break;
    if (!sign) {
      sign = { [TOKEN.S]: 1, [TOKEN.T]: -1 }[token];
    } else {
      value = (value << 1) + { [TOKEN.S]: 0, [TOKEN.T]: 1 }[token];
    }
  }
  return { token: paramStr, value: sign * value };
}

function replaceLabelWS(label) {
  return label.replace(/ /g, "s").replace(/\t/g, "t");
}

function asmObject(labels, mnemo, paramVal, paramLabel) {
  var replaceLabels = [];
  for (var i in labels) {
...
var ws = {
  ...
  compile: function (fullSource, lang) {
    WSUtil = WSUtilLoader(lang);
    TOKEN = lang.tokens;
    sourceTokens = { [TOKEN.S]: true, [TOKEN.N]: true, [TOKEN.T]: true };
    initWsKeywords();
    ...
    while (tokenizer.hasNext()) {
      ...
      debugToken += { [TOKEN.S]: "s", [TOKEN.T]: "t", [TOKEN.N]: "n" }[token];
...
function initWsKeywords() {
  ws.keywords = [
    ...
    {
      ws: TOKEN.S + TOKEN.N + TOKEN.S /*" \n "*/,
      mnemo: "dup",
      constr: ws.WsDouble,
      param: null,
    },
    {
      ws: TOKEN.S + TOKEN.T + TOKEN.S /*" \t "*/,
      mnemo: "copy",
      constr: ws.WsCopyNth,
      param: "NUMBER",
    },
    {
      ws: TOKEN.S + TOKEN.N + TOKEN.T /*" \n\t"*/,
      mnemo: "swap",
      constr: ws.WsSwapTop,
      param: null,
    },
    {
      ws: TOKEN.S + TOKEN.N + TOKEN.N /*" \n\n"*/,
      mnemo: "drop",
      constr: ws.WsDropTop,
      param: null,
    },
    {
      ws: TOKEN.S + TOKEN.T + TOKEN.N /*" \t\n"*/,
      mnemo: "slide",
      constr: ws.WsSlide,
      param: "NUMBER",
    },
    {
      ws: TOKEN.T + TOKEN.S + TOKEN.S + TOKEN.S /*"\t   "*/,
      mnemo: "add",
      constr: ws.WsAddition,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.S + TOKEN.S + TOKEN.T /*"\t  \t"*/,
      mnemo: "sub",
      constr: ws.WsSubtraction,
      param: null,
    },
    {
      ws: TOKEN.T * TOKEN.S + TOKEN.S + TOKEN.N /*"\t  \n"*/,
      mnemo: "mul",
      constr: ws.WsMultiplication,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.S + TOKEN.T + TOKEN.S /*"\t \t "*/,
      mnemo: "div",
      constr: ws.WsIntDivision,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.S + TOKEN.T + TOKEN.T /*"\t \t\t"*/,
      mnemo: "mod",
      constr: ws.WsModulo,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.T + TOKEN.S /*"\t\t "*/,
      mnemo: "store",
      constr: ws.WsHeapStore,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.T + TOKEN.T /*"\t\t\t"*/,
      mnemo: "retrieve",
      constr: ws.WsHeapRetrieve,
      param: null,
    },
    {
      ws: TOKEN.N + TOKEN.S + TOKEN.S /*"\n  "*/,
      mnemo: "label",
      constr: ws.WsLabel,
      param: "LABEL",
    },
    {
      ws: TOKEN.N + TOKEN.S + TOKEN.T /*"\n \t"*/,
      mnemo: "call",
      constr: ws.WsCall,
      param: "LABEL",
    },
    {
      ws: TOKEN.N + TOKEN.S + TOKEN.N /*"\n \n"*/,
      mnemo: "jmp",
      constr: ws.WsJump,
      param: "LABEL",
    },
    {
      ws: TOKEN.N + TOKEN.T + TOKEN.S /*"\n\t "*/,
      mnemo: "jz",
      constr: ws.WsJumpZ,
      param: "LABEL",
    },
    {
      ws: TOKEN.N + TOKEN.T + TOKEN.T /*"\n\t\t"*/,
      mnemo: "jn",
      constr: ws.WsJumpNeg,
      param: "LABEL",
    },
    {
      ws: TOKEN.N + TOKEN.T + TOKEN.N /*"\n\t\n"*/,
      mnemo: "ret",
      constr: ws.WsReturn,
      param: null,
    },
    {
      ws: TOKEN.N + TOKEN.N + TOKEN.N /*"\n\n\n"*/,
      mnemo: "end",
      constr: ws.WsEndProgram,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.N + TOKEN.S + TOKEN.S /*"\t\n  "*/,
      mnemo: "printc",
      constr: ws.WsPrintChar,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.N + TOKEN.S + TOKEN.T /*"\t\n \t"*/,
      mnemo: "printi",
      constr: ws.WsPrintNum,
      param: null,
    },
    {
      ws: TOKEN.T + TOKEN.N + TOKEN.T + TOKEN.S /*"\t\n\t "*/,
      mnemo: "readc",
      constr: ws.WsReadChar,
      param: null,
    },
    {
      ws: TOKEN.T * TOKEN.N + TOKEN.T + TOKEN.T /*"\t\n\t\t"*/,
      mnemo: "readi",
      constr: ws.WsReadNum,
      param: null,
    },
